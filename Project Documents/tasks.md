...
3. [ ] Establish a network packet registry with protocol versioning for minimal client features (HUD, compass, puzzle UIs) and future expansion; add handshake/version guard.
4. [ ] Introduce a central config class (server + client) for corruption toggles, entrance search radii, generation limits, debug flags (per 01 §6, 02 §6, 90 §5/§7).
...
11. [ ] Extend fragment item model to carry IDs/type/value/rarity/affinity/conflicts (align with example JSON in 02 §4) and register via datapack.
12. [ ] Update the UI flow: 3×3 grid, explicit Synthesize button, server validates and consumes; show preview + server-sourced result (per 02 §4 UI/MVP).
...
19. [ ] Replace MissionGatewayBlock stub with interaction flow: verify bound signal, call RunRegistry.createRun, teleport party, consume signal, and despawn entrance (per 02 §5).
...
21. [ ] Implement RunRegistry: allocate RunId, form RunParty, build RunConfig from Mission Signal payload, and create ActiveRun/RunState (per 90 §3 Iteration B).
...
23. [ ] Implement WorldspaceManager: allocate per-run AABB, apply chunk tickets limits, stamp entry pad, track occupied regions, and cleanup (air wipe, despawns, ticket release) (90 §3, §6 Risks).
24. [ ] Add commands: /shadowmission seed <id> (print config), /shadowmission abort <id> (cleanup) (90 §3, §7/§8).

Dungeon Generation & Content
25. [ ] Create datapack scaffolding directories: rooms/, rosters/, puzzles/, bosses/, affixes/, themes/, structures/, signals/, loot_tables/, advancements/, tags/ (per 02 §3, 90 §4).
26. [ ] Implement a small room set (3–6 rooms) for one theme (e.g., Grave) with NBT templates, sockets/bounds/weights.
27. [ ] Build a generator pipeline MVP: graph selection (chain/T-branch), spatial placement with backtracking and AABB checks, corridors/doors rules (per 90 §3 Iteration C).
...
UX & HUD
33. [ ] Enrich Mission HUD with timer, theme/tier/affixes, keys/obelisks, compass hint, and party status (90 §3 Iteration E).
...
36. [ ] Implement Shadow aspect data model + sync utilities per 01 §4: corruption float (0–1), shadow_state boolean; expose get/set helpers and display mapping 0–100.
...
51. [ ] Add robust error paths: missing missions dimension, failed entrance placement, invalid run state; surface actionable errors to players and logs (02 §5 step 4).
...
53. [ ] Add null-safety and server-side checks throughout BE/blocks (e.g., SignalLocatorBlockEntity recomputePreview already guards; extend with validation results to client).
